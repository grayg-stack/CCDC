#!/bin/bash
###############################################################################
# audit_vm1_ubuntu_ecom.sh
# MACCDC 2026 - VM1: Ubuntu Ecom (Ubuntu Server 24.04.3)
# IP: 172.20.242.30 | Scored: HTTP(80), HTTPS(443)
#
# Interactive user audit script. Lists all users, shows details, and prompts
# the operator for actions (password change, delete, lock, skip).
#
# Usage: sudo ./audit_vm1_ubuntu_ecom.sh
###############################################################################
...
LOGFILE="${LOGDIR}/audit_vm1_$(date +%Y%m%d_%H%M%S).log"
mkdir -p "$LOGDIR"
chmod 700 "$LOGDIR"
...
echo -e "${BOLD}============================================================${NC}"
echo -e "${BOLD} VM1 Ubuntu Ecom - Interactive User Audit${NC}"
echo -e "${BOLD} $(date)${NC}"
echo -e "${BOLD}============================================================${NC}"
...
# Collect users: UID >= 1000 or has a real login shell
AUDIT_USERS=()
while IFS=: read -r username _ uid gid _ homedir shell; do
    # Include if UID >= 1000 or shell is a login shell (not nologin/false)
    if [[ $uid -ge 1000 ]] || { [[ "$shell" != */nologin ]] && [[ "$shell" != */false ]] && [[ "$shell" != "/bin/sync" ]] && [[ $uid -ne 0 ]]; }; then
        AUDIT_USERS+=("$username")
    fi
done < /etc/passwd

# Also always include root for display
...
if [[ -n "$docker_users" ]]; then
    ...
    echo -e "${GREEN}[*] No users in docker group (or docker group does not exist).${NC}"
fi
echo ""
...
for username in "${AUDIT_USERS[@]}"; do
    # Skip root (handled separately above)
    user_info=$(getent passwd "$username" 2>/dev/null) || continue
    uid=$(echo "$user_info" | cut -d: -f3)
    shell=$(echo "$user_info" | cut -d: -f7)
...
echo ""
echo "REMINDERS:"
echo "  - Verify scored services (HTTP/HTTPS) are still working!"
